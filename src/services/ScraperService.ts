...
const getProductsFromRestaurantCookie = async (referrer: string, menuID: number) => {
    const headers = {...defaultHeaders, Cookie: cookie, referrer: referrer };
    // console.log(cookie);
    const data = await axios.get(`https://www.pedidosya.com.uy/v2/niles/partners/${menuID}/menus?isJoker=false&occasion=DELIVERY`, {
        headers: headers,
    });
...
const getMultipleRestaurantProducts = async (cookies: string[]) => {
    const promises = cookiesPrueba.map( async (menuID: number) => {
        return await getProductsFromRestaurantCookie("https://www.pedidosya.com.uy/restaurantes/montevideo/mila-house-menu?origin=shop_list", menuID);
    });
...
const getAllRestaurantsInArea = async () => {
    const max = 500;
    const offset = 0;
    
    // yo
    // const lat = -34.8982033;
    // const long = -56.1707867;
...
export default {
    getProductsFromRestaurantCookie,
    getMultipleRestaurantProducts,
